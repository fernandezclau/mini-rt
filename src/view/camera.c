 ...
 * 
 * @param cam Pointer to the camera structure with direction and identity basis vectors.
 */
void compute_camera_basis(camera *cam)
{
    ...
    cam->identity[0] = rotate_vect(cam->identity[0], axis, cos_theta, sin_theta);
    cam->identity[1] = rotate_vect(cam->identity[1], axis, cos_theta, sin_theta);
    cam->identity[2] = rotate_vect(cam->identity[2], axis, cos_theta, sin_theta);
}
